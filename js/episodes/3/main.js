...
function toggleQuestion(element) {
	...
	if (health === '-1' || health === '-10') {
		...
		_health.classList.add('b-stat_health');
		_health.classList.add('b-stat_minus');
		_health.innerText = '-10';
	} else if (health === '1' || health === '10') {
		var _health2 = block.querySelector('.js-health');
		_health2.classList.add('b-stat_health');
		_health2.classList.add('b-stat_plus');
		_health2.innerText = '+10';
	}

	if (power === '-1' || power === '-10') {
		var _power = block.querySelector('.js-power');
		_power.classList.add('b-stat_power');
		_power.classList.add('b-stat_minus');
		_power.innerText = '-10';
	} else if (power === '1' || power === '10') {
		var _power2 = block.querySelector('.js-power');
		_power2.classList.add('b-stat_power');
		_power2.classList.add('b-stat_plus');
